...
createCookie("TOL_Language", "tr", 90);
    /* jQuery
    ``````````````````````````````````````````````````````````````````````````` */
...
(function($){
	...
	$.fn.hsSlide  = function(options){
		...
		function scroll_to(current_page){
			...
			elements_wrapper.scrollTo(elements.eq(elements_scolling_index), 500, { axis:'x' });
		}
	...
	$.fn.getMaxHeight = function(){
		return ( Math.max.apply(this, $.map( this , function(e){ return $(e).height() }) ) );
	};
